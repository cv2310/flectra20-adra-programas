        ...
        if report_name == self.RENDICION_CUENTAS:
            if sql_nro == 1:
                ...
                if date_close < date_from:
                    ...
                    sqlHead = self.getSqlHead().format(sqlFiledBase= sqlFiledBase, sqlOpen=sqlOpen, unionAll='UNION ALL',sqlClose='{sqlClose}')
                    sqlWhere = f''' x_account_analytic_account_id = {x_account_analytic_account_id}\
                                            AND date <='{date_close}'  \
                                            '''
                else:
                    sqlHead = self.getSqlHead().format(sqlFiledBase=sqlFiledBase, sqlOpen='', unionAll='',sqlClose='{sqlClose}')
                    sqlWhere = f''' x_account_analytic_account_id = {x_account_analytic_account_id}\
                                            AND date <'{date_from}'  \
                                            '''
  ...
  #     if total_income and total_expenses:
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if x_report_version == "new":
            ...
            if x_document_type in ['INGRESO', 'EGRESO']:
                accounts_res = self.with_context(data['form'].get('used_context', {}))._get_mayor_ledger_entries(
                    accounts, x_account_analytic_account_id[0], x_sort_by, x_report_type, x_document_type)
            elif x_document_type == 'BANCO':
                accounts_res = self.with_context(data['form'].get('used_context', {}))._get_bank_ledger_entries_old(
                    accounts, x_account_analytic_account_id[0], x_sort_by, x_report_type, x_document_type)
            elif x_document_type == 'CONCILIACION':
                accounts_res = self.with_context(data['form'].get('used_context', {}))._get_bank_reconciliation_entries_old(
                    accounts, x_account_analytic_account_id[0], x_sort_by, x_bank_final_balance)
            elif x_document_type == 'RENDICION':
                accounts_res = self.with_context(data['form'].get('used_context', {}))._get_accountability_entries_old(
                    accounts, x_account_analytic_account_id[0], x_sort_by, x_report_type, x_document_type)
...
# Anterior
    ...
    def _get_bank_reconciliation_entries_old(self, accounts, x_account_analytic_account_id, x_sort_by, x_bank_final_balance):
        cr = self.env.cr
        MoveLine = self.env['account.move.line']
        move_lines = {x: [] for x in accounts.ids}
